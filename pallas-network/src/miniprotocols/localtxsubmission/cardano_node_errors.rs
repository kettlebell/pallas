...
use pallas_codec::minicbor::{self, data::{Tag, Type}, decode, decode::{Error, Token}, Decode, Decoder, Encode};
use pallas_codec::utils::{AnyUInt, Bytes};
use pallas_utxorpc::TxHash;
use crate::miniprotocols::localstate::queries_v16::Coin;
use crate::miniprotocols::localtxsubmission::cardano_node_errors::ConwayCertsPredFailure::WithdrawalsNotInRewardsCERTS;
use super::codec::NodeErrorDecoder;

/// Represents the top-level error response from the cardano node after use has submitted a TX.
...
pub struct ApplyTxError {
    /// List of individual errors from the node.
...
impl Decode<'_, NodeErrorDecoder> for ApplyTxError {
    fn decode(d: &mut Decoder, ctx: &mut NodeErrorDecoder) -> Result<Self, Error> {
        ...
        // The Cardano era tag, currently 6 for Conway.
        let _era_tag = expect_u8(d, ctx)?;

        // Here we expect a definite array or variable size
        let num_errors = expect_definite_array(vec![], d, ctx, "ApplyTxError_3")?;
        // This top level array can be popped off the context stack, since we'll explicitly handle
        // each error in the loop below.
        assert_eq!(
        ...
        for _ in 0..num_errors {
            match ConwayLedgerPredFailure::decode(d, ctx) {
                ...
                Err(e) => {
                    // If an end-of-input error is returned, it means that the current bytes
                    // returned from the node is incomplete.
                    if e.is_end_of_input() {
                        return Err(e);
                    }
                }
...
impl<'b, K: pallas_codec::minicbor::Decode<'b, NodeErrorDecoder>, V: pallas_codec::minicbor::Decode<'b, NodeErrorDecoder>>
Decode<'b, NodeErrorDecoder> for OHashMap<K, V>
{
    fn decode(d: &mut Decoder<'b>, ctx: &mut NodeErrorDecoder) -> Result<Self, decode::Error> {
        let v: Result<Vec<(K, V)>, _> = d.map_iter_with::<NodeErrorDecoder, K, V>(ctx)?.collect();
...
impl Decode<'_, NodeErrorDecoder> for ConwayCertsPredFailure {
    fn decode(d: &mut Decoder<'_>, ctx: &mut NodeErrorDecoder) -> Result<Self, Error> {
        expect_definite_array(vec![2, 3], d, ctx, "ConwayCertsPredFailure")?;
        match expect_u8(d, ctx) {
            Ok(tag) => match tag {
                0 => {
                    match OHashMap::decode(d, ctx) {
                        Ok(withdraw_map) => {
                            Ok(WithdrawalsNotInRewardsCERTS(withdraw_map))
                        }
                        Err(_) => Err(Error::message(format!(
                            "WithdrawalsNotInRewardsCERTS decoding failed {}",
                            tag
                        )))
                    }
                }
                _ => Err(Error::message(format!(
                    "ConwayCertsPredFailure unhandled tag {}",
                    tag
                )))
            }
            Err(e) => {
                if e.is_end_of_input() {
                    ...
                    Err(Error::message(
                        "ConwayCertsPredFailure::decode: expected tag",
                    ))
                }
            }
        }
...
#[derive(Debug, Clone)]
/// Type that represents individual ledger errors.
/// See https://github.com/IntersectMBO/cardano-ledger/blob/492d54ca8cf9a43b7df9d6c183c88632f72b2cde/eras/conway/impl/src/Cardano/Ledger/Conway/Rules/Ledger.hs#L135
///
/// NOTE: when attempting to decode bytes into this type, there are 2 possible errors:
///  1. The byte buffer is incomplete. An end-of-input error is returned here and we need to wait
///     for additional bytes from the node before we can try decoding again.
///  2. The inner error type is not currently handled by the decoder. In this case we consume all
///     the bytes relating to this unhandled error and return an Error::message(_)
...
impl Decode<'_, NodeErrorDecoder> for ConwayLedgerPredFailure {
    fn decode(d: &mut Decoder, ctx: &mut NodeErrorDecoder) -> Result<Self, Error> {
        if let Err(e) = expect_definite_array(vec![2, 3], d, ctx, "ConwayLedgerPredFailure") {
            ...
            clear_bytes_in_unhandled_error(d, ctx)?;
        }
        match expect_u8(d, ctx) {
            Ok(tag) => {
                match tag {
                    1 => {
                        match ConwayUtxowPredFailure::decode(d, ctx) {
                            Ok(utxow_failure) => {
                                Ok(ConwayLedgerPredFailure::UtxowFailure(utxow_failure))
                            }
                            Err(e) => {
                                if e.is_end_of_input() {
                                    Err(e)
                    ...
                    2 => {
                        match ConwayCertsPredFailure::decode(d, ctx) {
                            Ok(certs_failure) => {
                                Ok(ConwayLedgerPredFailure::CertsFailure(certs_failure))
                            }
                            Err(e) => {
                                if e.is_end_of_input() {
                                    Err(e)
                    ...
                    _ => {
                        clear_bytes_in_unhandled_error(d, ctx)?;
                        Err(Error::message("not ConwayLedgerPredFailure"))
                    }
                }
            }
            Err(e) => {
                if e.is_end_of_input() {
                    ...
                    clear_bytes_in_unhandled_error(d, ctx)?;
                    Err(Error::message(
                        "ConwayLedgerPredFailure::decode: expected tag",
                    ))
                }
            }
        }
...
// NOTE: -------------------------------------------------------------------------------------------
// For all `Decode` impls below:
//
// If we run into an end-of-input error during the decoding process we simply bubble-up this same
// error. Otherwise if the error type is not currently handled we return an Error::message(_)
// instance (the bytes relating to this unhanlded error will be consumed by
// `ConwayLedgerPredFailure::decode()` above).
// -------------------------------------------------------------------------------------------------

/// A UTxO error.
/// https://github.com/IntersectMBO/cardano-ledger/blob/492d54ca8cf9a43b7df9d6c183c88632f72b2cde/eras/conway/impl/src/Cardano/Ledger/Conway/Rules/Utxo.hs#L78
    ...
    fn decode(d: &mut Decoder, ctx: &mut NodeErrorDecoder) -> Result<Self, Error> {
        ...
        match expect_u8(d, ctx) {
            Ok(tag) => match tag {
                ...
                _ => Err(Error::message("not ConwayUtxoPredFailure")),
            },
            Err(e) => {
                if e.is_end_of_input() {
                    Err(e)
                    ...
                    add_collection_token_to_context(d, ctx)?;
                    Err(Error::message(
                        "ConwayUtxoPredFailure::decode: expected tag",
                    ))
                }
            }
        }
...
/// A UTxO script error.
/// https://github.com/IntersectMBO/cardano-ledger/blob/492d54ca8cf9a43b7df9d6c183c88632f72b2cde/eras/conway/impl/src/Cardano/Ledger/Conway/Rules/Utxos.hs#L74
#[derive(Debug, Clone)]
...
/// https://github.com/IntersectMBO/cardano-ledger/blob/492d54ca8cf9a43b7df9d6c183c88632f72b2cde/eras/conway/impl/src/Cardano/Ledger/Conway/Rules/Utxow.hs#L94
#[derive(Debug, Clone)]
    ...
    fn decode(d: &mut Decoder, ctx: &mut NodeErrorDecoder) -> Result<Self, Error> {
        ...
        match expect_u8(d, ctx) {
            Ok(tag) => match tag {
                ...
                _ => Err(Error::message(format!(
                    "ConwayUtxowPredFailure unhandled tag {}",
                    tag
                ))),
            },
            Err(e) => {
                if e.is_end_of_input() {
                    ...
                    add_collection_token_to_context(d, ctx)?;
                    Err(Error::message(
                        "ConwayUtxoPredwFailure::decode: expected tag",
                    ))
                }
            }
        }
        ...
        Ok(dt) => {
            match dt {
                ...
                _ => Err(minicbor::decode::Error::message(
                    "unknown cbor data type for Conway Value enum",
                )),
            }
        }
        Err(e) => {
...
/// This function consumes all bytes in the decoder relating to an unhandled node error.
fn clear_bytes_in_unhandled_error(
    decoder: &mut Decoder,
    ctx: &mut NodeErrorDecoder,
) -> Result<(), Error> {
...
mod tests {
    ...
    // Example error response from the node. Contains 5 errors and we currently can decode 2 of them
    const NON_SCRIPT_ERRORS: &str = "";

    #[test]
    fn test_decode_non_script_error() {
        let bytes = hex::decode(format!(
            "{}{}",
            generate_conway_error_prefix(5).unwrap(),
            NON_SCRIPT_ERRORS
        ))
        .unwrap();
    ...
    fn test_decode_split_error() {
        let mut bytes = hex::decode(format!(
            "{}{}",
            generate_conway_error_prefix(5).unwrap(),
            NON_SCRIPT_ERRORS
        ))
        ...
        let mut cc = NodeErrorDecoder::new();
        let result = cc.try_decode_with_new_bytes(&bytes);
        matches!(result, Ok(DecodingResult::Incomplete));
    ...
    const CONWAY_WDRWL_ERROR_COMPLETE: &str = "";
    ...
    fn test_decode_conway_script_incomplete() {
        ...
        let mut cc = NodeErrorDecoder::new();
        let result = cc.try_decode_with_new_bytes(&bytes);
        assert!(matches!(result, Ok(DecodingResult::Incomplete)))
    }

    #[test]
    fn test_decode_conway_wdrwl_error() {
        let bytes = hex::decode(CONWAY_WDRWL_ERROR_COMPLETE).unwrap();

        let mut cc = NodeErrorDecoder::new();
        let result = cc.try_decode_with_new_bytes(&bytes);
        assert!(matches!(result, Ok(DecodingResult::Complete(_))));
    }

    #[test]
    fn test_decode_conway_script_error_complete() {
        let bytes = generate_single_script_error_cbor_bytes(65535); //hex::decode(COMPLETE_SCRIPT_ERROR).unwrap();

        let mut cc = NodeErrorDecoder::new();
        let result = cc.try_decode_with_new_bytes(&bytes);
        if let Ok(DecodingResult::Complete(Message::RejectTx(error))) = result {
            assert_eq!(error.node_errors.len(), 1);
        } else {
            panic!("ZZZ: {:?}", result);
        }
    }
    ...
    #[test]
    fn test_decode_conway_script_error_split() {
        let mut bytes = generate_single_script_error_cbor_bytes(65535); //hex::decode(COMPLETE_SCRIPT_ERROR).unwrap();
        let tail = bytes.split_off(3 * bytes.len() / 4);
    ...
    #[test]
    fn test_decode_combined_errors_complete() {
        let bytes = generate_combined_errors_cbor(65535);

        let mut cc = NodeErrorDecoder::new();
        let result = cc.try_decode_with_new_bytes(&bytes);
        if let Ok(DecodingResult::Complete(Message::RejectTx(error))) = result {
            assert_eq!(error.node_errors.len(), 3);
        } else {
            panic!("ZZZ: {:?}", result);
        }
    }
    ...
    #[test]
    fn test_decode_combined_errors_split() {
        let num_partitions = 10;
        for i in 1..num_partitions {
            let mut bytes = generate_combined_errors_cbor(65535); //hex::decode(COMPLETE_SCRIPT_ERROR).unwrap();
            let tail = bytes.split_off(i * bytes.len() / num_partitions);
    ...
    fn generate_combined_errors_cbor(script_byte_length: u32) -> Vec<u8> {
        let s = format!(
            "{}{}{}",
            generate_conway_error_prefix(6).unwrap(),
            NON_SCRIPT_ERRORS,
            generate_script_error_cbor(script_byte_length)
        );
        hex::decode(s).unwrap()
    }
    ...
    fn generate_random_hex_string(len: usize) -> String {
        ...
        // By restricting values to [0, 127], we generate random ASCII characters.
        (0..len)
            .map(|_| format!("{:02x}", rng.gen_range(0..128))) // Generate random numbers in the range [0, 15] and format them as hex
            .collect()
    }

    fn generate_script_error_cbor(length: u32) -> String {
        format!(
            "8201820082008300f5820181830179{:04x}{}59{:04x}{}",
            length,
            generate_random_hex_string(length as usize),
            length,
            generate_random_hex_string(length as usize),
        )
    }

    fn generate_single_script_error_cbor_bytes(length: u32) -> Vec<u8> {
        let length = u32::min(length, 65535);
        let s = format!(
            "{}{}",
            generate_conway_error_prefix(1).unwrap(),
            generate_script_error_cbor(length),
        );
        hex::decode(s).unwrap()
    }

    fn generate_conway_error_prefix(num_errors: u8) -> Option<String> {
        if num_errors == 0 || num_errors > 9 {
            return None;
        }

        Some(format!("82028182068{}", num_errors))
    }
}
